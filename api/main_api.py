...
import torch
import numpy as np
from fastapi import FastAPI, File, UploadFile, Form, WebSocket, BackgroundTasks, HTTPException
...
async def convert_voice(
    ...
    # Process audio
    try:
        await BackgroundTasks().add_task(process_audio, str(input_path), str(output_path), params)
    ...
    try:
        ...
        while True:
            ...
            if frame_count >= 10:  # Process every ~10 chunks
                ...
                # Process audio
                try:
                    result_path, _ = process_audio(str(temp_input), str(temp_output), params)
